...
HANDLE CreateAccessTokenFromPID(DWORD dwProcessId) {
    ...
    if (hProc) {
        ...
        if (OpenProcessToken(hProc, TOKEN_DUPLICATE, &hToken)) {
            ...
            if (!DuplicateTokenEx(hToken, TOKEN_ALL_ACCESS, &attrs, SecurityImpersonation,
                                  TokenImpersonation, &hDupToken)) {
                std::cout << "DuplicateTokenEx: "
        ...
        std::cout << "OpenProcess: " << std::system_category().message(GetLastError())
                  << std::endl;
    }
...
BOOL ImpersonateAsSystem() {
    ...
    if (dwPID != 0) {
        ...
        if (hToken != INVALID_HANDLE_VALUE) {
            ...
            if (!success) {
                std::cout << "ImpersonateLoggedOnUser: "
                          << std::system_category().message(GetLastError()) << std::endl;
            }
            CloseHandle(hToken);
        }
...
DWORD StartTIServiceAndGetPID() {
    ...
    if (hSCM) {
        SC_HANDLE hService =
            OpenService(hSCM, L"TrustedInstaller", SERVICE_START | SERVICE_QUERY_STATUS);
        if (hService) {
            ...
            std::cout << "OpenService: " << std::system_category().message(GetLastError())
                      << std::endl;
        }
        CloseServiceHandle(hSCM);
    } else {
        std::cout << "OpenSCManager: " << std::system_category().message(GetLastError())
                  << std::endl;
    }
